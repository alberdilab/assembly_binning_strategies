...
cluster_kegg_proportion_max %>%
  mutate(secondary_cluster=factor(secondary_cluster,levels=rev(cluster_tree$tip.label[cluster_tree$tip.label %in% cluster_kegg_proportion_max$secondary_cluster]))) %>%
   mutate(overall_strategy=factor(overall_strategy,levels=rev(strategies))) %>%
...
```{r function_recovery_vs_completeness, comment="", message=FALSE, warning=FALSE, fig.height=4, fig.width=10, fig.fullwidth=TRUE}
...
full_join(completeness_stats,functions_stats,by="overall_strategy") %>%
  mutate(overall_strategy=factor(overall_strategy,levels=strategies)) %>%
