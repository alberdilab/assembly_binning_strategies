...
```{r import_data, comment="", message=FALSE, warning=FALSE}
...
df_sum <- df_filt %>%
  ...
  mutate(strategy = case_when(str_detect(strategy, "coassembly_long") ~ "coassembly animal",
                              str_detect(strategy, "coassembly_treat") ~ "coassembly timepoint-all",
                              str_detect(strategy, "coassembly_cage_treat") ~ "multi-coverage binning timepoint-cage",
                              str_detect(strategy, "cobinning_long") ~ "multi-coverage binning animal",
                              str_detect(strategy, "cobinning_cage_treat") ~ "coassembly timepoint-cage",
                              str_detect(strategy, "vamb_long") ~ "multi-split animal",
                              str_detect(strategy, "vamb_cage_treat") ~ "multi-split timepoint-cage",
                              str_detect(strategy, "vamb_treat") ~ "multi-split timepoint-all",
                              str_detect(strategy, "individual") ~ "single-coverage binning",
                              ),
  )
...
df_sum_total_ind <- df_sum_total %>%
  filter(strategy == "single-coverage binning") %>%
...
indiv <- tibble(
  strategy = "single-coverage binning",
...
mean_coassembly_t <- coassembly_df %>%
  filter(job == "Assembly" & strategy == "coassembly_treat") %>%
  pull(time_s) %>%
  mean() / 30
mean_coassembly_ct <- coassembly_df %>%
  filter(job == "Assembly" & strategy == "coassembly_cage_treat") %>%
  pull(time_s) %>%
  mean() / 5
...
mean_binning_t <- coassembly_df %>%
  filter(job == "binning" & strategy == "coassembly_treat") %>%
  pull(time_s) %>%
  mean() / 30
mean_binning_ct <- coassembly_df %>%
  filter(job == "binning" & strategy == "coassembly_cage_treat") %>%
  pull(time_s) %>%
  mean() / 5
...
mean_refinement_t <- coassembly_df %>%
  filter(job == "refinement" & strategy == "coassembly_treat") %>%
  pull(time_s) %>%
  mean() / 30
mean_refinement_ct <- coassembly_df %>%
  filter(job == "refinement" & strategy == "coassembly_cage_treat") %>%
  pull(time_s) %>%
  mean() / 5
...
df_cob_cage_treat <- df_filt %>%
  filter(strategy == "cobinning_cage_treat")
...
df_ms_cage_treat <- df_filt %>%
  filter(strategy == "vamb_cage_treat")
...
checkm_t <- df_filt %>%
  filter(job == "checkm" & strategy == "vamb_treat")

checkm_ct <- df_filt %>%
  filter(job == "checkm" & strategy == "vamb_cage_treat")
...
vamb_multisplit_t <- df_filt %>%
  filter(job == "vamb_multisplit" & strategy == "vamb_treat")

vamb_multisplit_ct <- df_filt %>%
  filter(job == "vamb_multisplit" & strategy == "vamb_cage_treat")
...
mapping_t <- df_filt %>%
  filter(job == "assembly_mapping" & strategy == "vamb_treat")

mapping_ct <- df_filt %>%
  filter(job == "assembly_mapping" & strategy == "vamb_cage_treat")
